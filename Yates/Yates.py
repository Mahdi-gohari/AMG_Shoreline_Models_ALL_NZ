...
    plt.plot(predict_data.index, S_predict, label='Predicted Shoreline (2022 onwards, regression trend)', color='red', linestyle='--')
...
    plt.title('Shoreline Position: Observed, Modeled, and Predicted (Regression-Based Trend)')
    plt.legend()
    plt.grid(True)
...
    
    # Train a linear regression model to predict the trend based on wave energy
    X_train = train_data[['wave_energy']].values
    y_train = trend  # Trend from training period
    trend_model = LinearRegression()
    trend_model.fit(X_train, y_train)
    logging.info("Trained linear regression model for trend estimation")
    
    # Predict trend for the forecast period
    X_predict = predict_data[['wave_energy']].values
    trend_predict = trend_model.predict(X_predict)
...
    
    output_path = 'C:/Users/amgh628/Downloads/XGBoost/Wave4/Yates/Muriwai/Muriwai_smoothed_LR.csv'
    try:
